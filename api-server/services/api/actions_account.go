...
func isReservedAccountName(name string) bool {
	return strings.EqualFold(strings.TrimSpace(name), "Demo")
}
...
func handleAccountAction(actionPayload *ActionRequest, c *gin.Context, tracer *trace.Tracer, meter *metric.Meter, logger *slog.Logger) {
	...
	case "messaging_platform_update", "messagingplatforms_update":
		...
		c.JSON(200, resp)
		// Audit is persisted by account.UpdateMessagingPlatform (LogChange, DB) —
	...
	case "messaging_platform_delete", "messagingplatforms_delete":
		...
		c.JSON(200, resp)
		// Audit is persisted by account.DeleteMessagingPlatform (LogChange, DB) —
		// no MQ publish here to avoid a duplicate write.
		return
