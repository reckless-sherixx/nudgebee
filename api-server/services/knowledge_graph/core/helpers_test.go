...
// TestContributingSourcesFieldPropertyInvariant pins the post-NB-30989 rule:
// edge.ContributingSources (first-class field, persisted to the JSONB column)
// is the single source of truth. The legacy edge.Properties["contributing_sources"]
// slot must always project the same set of source names after dedup, so any
// downstream code reading from properties sees the same view as the column.
func TestContributingSourcesFieldPropertyInvariant(t *testing.T) {
	cases := []struct {
		name        string
		sources     []string
		wantSurvive string
	}{
		{"single source", []string{"manual"}, "manual"},
		{"manual beats traces; both contribute", []string{"manual", "traces"}, "manual"},
		{"k8s wins over manual; both contribute", []string{"manual", "k8s"}, "k8s"},
		{"three contributors; manual wins over traces+ebpf", []string{"manual", "traces", "ebpf"}, "manual"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			edges := make([]*DbEdge, 0, len(tc.sources))
			for _, src := range tc.sources {
				edges = append(edges, &DbEdge{
					...
					Source:            src,
					TenantID:          "t1",
					Properties:        map[string]interface{}{"latency_ms": 1},
				})
			}
			deduped := DeduplicateEdgesWithPriority(edges)
			if len(deduped) != 1 {
				t.Fatalf("dedup returned %d edges, want 1", len(deduped))
			}
			survivor := deduped[0]
			if survivor.Source != tc.wantSurvive {
				t.Errorf("survivor.Source = %q, want %q", survivor.Source, tc.wantSurvive)
			}

			fieldSources := make(map[string]bool, len(survivor.ContributingSources))
			for _, cs := range survivor.ContributingSources {
				if cs.Source == "" {
					t.Errorf("ContributingSources contains empty source name")
				}
				if fieldSources[cs.Source] {
					t.Errorf("ContributingSources duplicates source %q", cs.Source)
				}
				fieldSources[cs.Source] = true
			}
			for _, want := range tc.sources {
				if !fieldSources[want] {
					t.Errorf("ContributingSources missing %q; got %v", want, fieldSources)
				}
			}

			propSources, ok := survivor.Properties["contributing_sources"].([]string)
			if !ok {
				t.Fatalf("Properties[contributing_sources] not []string; got %T", survivor.Properties["contributing_sources"])
			}
			propSet := make(map[string]bool, len(propSources))
			for _, s := range propSources {
				propSet[s] = true
			}
			if len(propSet) != len(fieldSources) {
				t.Errorf("field vs property cardinality drift: field=%v property=%v", fieldSources, propSet)
			}
			for s := range fieldSources {
				if !propSet[s] {
					t.Errorf("property slot missing %q present in field; field=%v property=%v", s, fieldSources, propSet)
				}
			}
		})
	}
...
// TestSyncContributingSourcesPropertyEmptyField guards the fallback: when an
// edge has Source set but no ContributingSources entries (which happens for
// a freshly-constructed edge that hasn't gone through dedup yet),
// syncContributingSourcesProperty must still emit the winning source into the
// property slot so partial-construction paths don't write an empty list.
func TestSyncContributingSourcesPropertyEmptyField(t *testing.T) {
	edge := &DbEdge{
		Source:              "k8s",
		Properties:          map[string]interface{}{},
		ContributingSources: nil,
	}
	syncContributingSourcesProperty(edge)
	got, _ := edge.Properties["contributing_sources"].([]string)
	if len(got) != 1 || got[0] != "k8s" {
		t.Errorf("expected fallback to [k8s]; got %v", got)
	}

	// Also: an entry with an empty Source name must be filtered out, not
	// projected as "".
	edge2 := &DbEdge{
		Source: "manual",
		ContributingSources: []EdgeContributingSource{
			{Source: "manual", LastSeenAt: time.Now()},
			{Source: "", LastSeenAt: time.Now()},
		},
	}
	syncContributingSourcesProperty(edge2)
	got2, _ := edge2.Properties["contributing_sources"].([]string)
	if len(got2) != 1 || got2[0] != "manual" {
		t.Errorf("expected empty-source entries filtered; got %v", got2)
	}
}
