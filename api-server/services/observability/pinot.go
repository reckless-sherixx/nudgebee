...
func buildPinotSQL(table, tsCol, whereClause string, startMs, endMs int64, tsMode pinotTsMode, limit, offset int, sortFields []SortField) string {
	...
	tsColQ := pinotQuoteIdent(tsCol)
	// Build the time predicate per the bounds the caller supplied:
	//   - both unset → omit (logs_get_query preview without a global filter)
	//   - one set    → use >= / <= so a "since" or "until" query still matches
	//     (BETWEEN X AND 0 would zero-match — same bug class in a new shape)
	//   - both set   → BETWEEN
	var timeFilter string
	if startMs != 0 || endMs != 0 {
		if tsMode.IsString {
			...
			case startMs != 0 && endMs != 0:
				...
				timeFilter = fmt.Sprintf("%s BETWEEN '%s' AND '%s'", tsColQ, startStr, endStr)
			case startMs != 0:
				timeFilter = fmt.Sprintf("%s >= '%s'", tsColQ, time.UnixMilli(startMs).UTC().Format(tsMode.GoFormat))
			default:
				timeFilter = fmt.Sprintf("%s <= '%s'", tsColQ, time.UnixMilli(endMs).UTC().Format(tsMode.GoFormat))
			}
		} else {
			sf := tsMode.ScaleFactor
			if sf <= 0 {
			...
			case startMs != 0 && endMs != 0:
				timeFilter = fmt.Sprintf("%s BETWEEN %d AND %d", tsColQ, startMs/sf, endMs/sf)
			case startMs != 0:
				timeFilter = fmt.Sprintf("%s >= %d", tsColQ, startMs/sf)
			default:
				timeFilter = fmt.Sprintf("%s <= %d", tsColQ, endMs/sf)
			}
		}
	}
	...
	if where != "" {
		...
		sql = fmt.Sprintf("SELECT * FROM %s ORDER BY %s LIMIT %d", pinotQuoteIdent(table), orderBy, limit)
	}
	if offset > 0 {
