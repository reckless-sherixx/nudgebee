...
// resolveCostReportDefinition determines the CUR S3 location and format for an
// account. For auto-registered accounts (org or single) all CUR details are
// pre-populated from the CloudFormation callback and used directly; otherwise it
// falls back to the DescribeReportDefinitions API.
func resolveCostReportDefinition(ctx providers.CloudProviderContext, account providers.Account) (s3Bucket, region, pathPrefix, compression, reportVersion, reportName, timeUnit string, err error) {
	if account.Data != nil {
		accountData := map[string]any{}
		if perr := common.UnmarshalJson([]byte(*account.Data), &accountData); perr != nil {
			ctx.GetLogger().Error("unable to parse accountData", "error", perr)
		}
...
// billing periods (months) that actually have a YYYYMMDD-YYYYMMDD/ folder,
// limited to at most the last maxMonths trailing months (including the current
// month). Folders outside that window or in the future are ignored. On a
// listing error it returns the error; the caller falls back gracefully.
func discoverAvailableBillingPeriods(ctx providers.CloudProviderContext, account providers.Account, maxMonths int) ([]providers.UsageReportPeriod, error) {
	if maxMonths <= 0 {
		return nil, nil
	}
	...
	s3Bucket, region, pathPrefix, _, _, _, _, err := resolveCostReportDefinition(ctx, account)
	if err != nil {
		return nil, err
	}
	...
	// Oldest month we are willing to backfill (first day of month, maxMonths-1 months back).
	now := time.Now().UTC()
	...
	for paginator.HasMorePages() {
		...
		if err != nil {
			return nil, fmt.Errorf("aws: failed to list CUR billing periods for bucket %q prefix %q: %w", s3Bucket, pathPrefix, err)
		}
		for _, cp := range page.CommonPrefixes {
			if cp.Prefix == nil {
				continue
			}
			start, ok := parseBillingPeriodStart(*cp.Prefix, pathPrefix)
			if !ok {
				continue
			}
			// Within the trailing window and not in the future.
			if start.Before(oldestAllowed) || start.After(currentMonthStart) {
				continue
			}
			...
			seen[key] = struct{}{}
			periods = append(periods, providers.UsageReportPeriod{Month: start.Month(), Year: start.Year()})
		}
	}

	slices.SortFunc(periods, func(a, b providers.UsageReportPeriod) int {
		if a.Year != b.Year {
	...
	ctx.GetLogger().Info("aws: discovered CUR billing periods for backfill",
		"bucket", s3Bucket, "prefix", pathPrefix, "count", len(periods), "maxMonths", maxMonths)
	return periods, nil
}

// parseBillingPeriodStart extracts the billing-period start date from a CUR
// folder common-prefix of the form "<pathPrefix>YYYYMMDD-YYYYMMDD/". Returns
// false if the trailing segment does not match the expected legacy CUR layout.
func parseBillingPeriodStart(commonPrefix, pathPrefix string) (time.Time, bool) {
	segment := strings.TrimSuffix(strings.TrimPrefix(commonPrefix, pathPrefix), "/")
