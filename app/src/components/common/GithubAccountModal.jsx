          ...
          {authType === 'github-app' && !isEdit && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Button
                ...
                onClick={() => {
                  setIsSubmitting(true);
                  // Open our own install endpoint (same-origin, so the session
                  // cookie is present); it signs the user's identity into the
                  // OAuth `state` and redirects the popup to GitHub. The
                  // callback then identifies the user from `state`, not the
                  // cookie — which GitHub's cross-site redirect would drop.
                  const popup = window.open('/api/integrations/github/install', 'github-auth', 'width=600,height=700,scrollbars=yes,resizable=yes');
