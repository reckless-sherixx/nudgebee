...
import CustomizedSlider from '@components/k8s/common/Slider';
      ...
      if (!nodeMap.has(id)) {
        ...
        const labelJSX = (
          <div className='sm-node'>
            <div aria-label={nodeData.Id.name} style={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-space-1)' }}>
      ...
      f?.Upstreams?.forEach((u) => {
        ...
        if (parts.length >= 3) {
          ...
          links.push({
            ...
            type: 'custom-edge',
            animated: true,
            markerEnd: { type: 'arrow', color: 'var(--ds-red-500)' },
            label: u.Protocol || '',
            data: { ...u },
          });
        }
              ...
              {!appName && (
                <>
                  ...
                  >
                    ...
                    <Switch checked={showOnlyErrorEdges} onChange={(_e, checked) => setShowOnlyErrorEdges(checked)} size='sm' />
                  </Box>
                  <Tooltip
