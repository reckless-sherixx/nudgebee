...
const KubernetesPodYaml: React.FC<KubernetesPodYamlProps> = ({ accountId, query, showEditButton = false }) => {
      ...
      <>
        {showEditButton && hasWriteAccess() && (
          <div style={{ marginBottom: 'var(--ds-space-2)' }}>
            <DsButton tone='secondary' onClick={() => setAllowEdit(true)} id='allow-edit' type='button'>
              Edit Yaml
            </DsButton>
          </div>
        )}
        ...
        {allowEdit && showEditButton && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: 'var(--ds-space-2)' }}>
              <DsButton tone='secondary' onClick={() => setAllowEdit(false)} id='cancel' type='button'>
                Cancel
              </DsButton>
              <DsButton tone='primary' onClick={() => handleSubmitOfEdit()} id='submit' type='button'>
                Submit
              </DsButton>
            </div>
          </div>
  ...
  return (
    <div style={{ paddingLeft: 'var(--ds-space-4)' }}>
      <Typography sx={{ paddingTop: 'var(--ds-space-2)', paddingBottom: 'var(--ds-space-4)' }}>{fileName}</Typography>
      {renderContent()}
    </div>
  );
};
