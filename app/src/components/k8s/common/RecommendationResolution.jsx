...
const RecommendationResolutionStatusDetails = function (_accountId, drilldownQuery, _row) {
  ...
  if (!statusMessage) {
    return <Typography sx={{ fontSize: 'var(--ds-text-body-lg)', color: ds.gray[700] }}>No details available</Typography>;
  }
        ...
        if (containsLink(resolution?.type_reference_id)) {
          referenceObj['component'] = (
            <Link
              ...
              openInNew
              style={{ fontSize: 'var(--ds-text-body)', fontWeight: 'var(--ds-font-weight-regular)' }}
            >
          ...
          referenceObj['text'] = (
            <Typography sx={{ fontSize: 'var(--ds-text-body)', fontWeight: 'var(--ds-font-weight-regular)', color: ds.gray[700] }}>
              {resolution?.type_reference_id}
            </Typography>
          );
        }
