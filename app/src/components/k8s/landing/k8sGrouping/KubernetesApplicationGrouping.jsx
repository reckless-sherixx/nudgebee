        ...
        return [
          {
            component: (
              <>
                <Link href={`/grouping?groupId=${item.id}`}>
                  <Text value={item.name} sx={{ color: 'var(--ds-blue-500)' }} />
                </Link>
  ...
  const tooltipConfig = useMemo(
    () => ({
      ...
      styles: {
        ...
        title: { fontWeight: 'bold', marginBottom: ds.space[2] },
        header: {
          display: 'grid',
          gridTemplateColumns: '1fr 1fr 1fr 1fr',
          fontWeight: 'bold',
          borderBottom: '1px solid var(--ds-brand-200)',
