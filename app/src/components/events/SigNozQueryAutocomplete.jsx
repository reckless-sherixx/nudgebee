...
const SigNozQueryAutocomplete = ({ accountId, onQueryChange, queryItems }) => {
  ...
  return (
    <Box sx={{ width: '100%', mx: 'auto', minHeight: '20vh' }}>
      ...
      <Box sx={{ position: 'relative', display: 'flex', alignItems: 'flex-start', gap: ds.space[2], width: '55%' }}>
        <TextField
          inputRef={inputRef}
          fullWidth
                  ...
                  >
                    {chips.slice(2).map((chip, index) => (
                      ...
                      >
                        ...
                        >
                          <CloseIcon sx={{ fontSize: 'var(--ds-text-small)' }} />
                        </IconButton>
                      </Box>
                    ))}
                  </Box>
      ...
      <Popover
        ...
        PaperProps={{
          sx: { maxWidth: ds.space.mul(0, 200), p: ds.space[4] },
        }}
      >
