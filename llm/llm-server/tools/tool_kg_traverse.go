...
func (t KGTraverseTool) Description() string {
	return `Traverse the infrastructure knowledge graph to explore dependencies, hosting, connectivity, and service call chains. ` +
		...
		`Pods are leaves: when a K8s Node or Workload is in the traversed set, its current Pods (RUNS_ON / MANAGES) are read live from k8s_pods and included; you cannot use a Pod as a seed of a multi-hop traversal, but kg_search_nodes+kg_get_node still work on Pod IDs. ` +
